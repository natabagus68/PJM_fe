...
export default function ApprovalReport() {
  const approve = useApprovalModel()
  const detail = useShowDetailModel()
  const choose = useChooseInspectionModel()
  return (
    <div>
      <div className="mb-5">
        <Breadcrumbs items={['Approve', 'Approve E-sign']} />
      </div>
        ...
        <div className="flex gap-3 flex-col">
          <div className="w-[370px] px-4 py-7 mt-4 border rounded-lg bg-[#FFFFFF] font-[400] text-[#514E4E]">
            <div className="flex justify-between mb-4 items-center">
              <h1>Status</h1>
              <span className="px-6 py-2 rounded-full text-[#FFFFFF] bg-[#F79009]">{approve.data.detail?.status}</span>
            </div>
            <div className="flex justify-between mb-4 items-center">
              <h1>Inspection ID</h1>
              <span className="font-[700]">{approve.data.detail?.inspectionId}</span>
            </div>
            <div className="flex justify-between mb-4 items-center">
              <h1>Inspection Date</h1>
              <span className="font-[700]">{approve.data.detail?.inspectionDate}</span>
            </div>
            <div className="flex justify-between mb-4 items-center">
              <h1>Confirmation Date</h1>
              <span className="font-[700]">{approve.data.detail?.confirmationDate}</span>
            </div>
            <div className="flex justify-between mb-4 items-center">
              <h1>Customer</h1>
              <span className="font-[700]">{approve.data.detail?.customer}</span>
            </div>
            <h1
          ...
          <div className="w-[370px] px-4 py-7 bg-[#FFFFFF] rounded-lg border">
            ...
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Machine Type</h1>
              <span className="font-[700]">{approve.data.result?.machineType}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Serial No</h1>
              <span className="font-[700]">{approve.data.result?.serialNo}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>MFG Date</h1>
              <span className="font-[700]">{approve.data.result?.mfgDate}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Capacity</h1>
              <span className="font-[700]">{approve.data.result?.capacity}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Slide Stroke</h1>
              <span className="font-[700]">{approve.data.result?.slideStroke}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Stroke Per Minutes</h1>
              <span className="font-[700]">{approve.data.result?.strokePerMinutes}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Die Height</h1>
              <span className="font-[700]">{approve.data.result?.dieHeight}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Slide Adjustment</h1>
              <span className="font-[700]">{approve.data.result?.slideAdjustment}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Dimensti Area Bloaster</h1>
              <span className="font-[700]">{approve.data.result?.dimensiAreaBloster}</span>
            </div>
            <div className="flex justify-between mb-4 items-center font-[400]">
              <h1>Dimensti Area Slide</h1>
              <span className="font-[700]">{approve.data.result?.dimensiAreaSlide}</span>
            </div>
          </div>
        </div>
        <div className="w-full mt-4 rounded-lg bg-[#FFFFFF] border">
          <div className="grid grid-cols-5 mt-3 border-b mx-6">
            {approve.menu?.map(item => (
              <NavLink
                key={item?.id}
                onClick={() => approve.handleSetNav()}
                className={`flex justify-center text-sm pb-2 mt-6 hover:pb-0 hover:border-b-2 hover:border-b-[#14988B] hover:text-[#14988B] text-[#6F6C6C] cursor-pointer
                  ${approve.nav ? "border-b-[#14988B]" : "text-[#6F6C6C]"}
                `}
                to={item?.to}
              >
                {item?.label}
          ...
          <Outlet />
            ...
            <div className="flex border-t mt-4 py-[16px] px-[32px] bg-[#FFFFFF] justify-end">
              <button
                ...
                onClick={(e) => approve.handleConfirmation(e)}
                >
                <ConfirmCheck />
                <span>Confirm</span>
                </button>
            </div>
          <ModalConfirm open={approve.open} setOpen={approve.setOpen} setOpenSuccess={approve.open} cb={() => alert("complete")} />
